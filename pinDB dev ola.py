 # -*- coding: utf-8 -*-
"""
Created on Tue Jul 18 10:09:10 2023
...
#    filename = "/Lufeng.fs"
    ...
    try:
        ...
        for key in root:
            ...
            CableDict[key][1] = Core Configuration - If available else "NO CORE CONFIGURATION YET"
            CableDict[key][2] = connection2
            CableDict[key][3] = Start of LSignal1..40
            CableDict[key][43] = Start of LTermination1..40
            CableDict[key][83] = Start of LSCR1..40
            CableDict[key][123] = Start of Color1..40
            CableDict[key][163] = Start of RSCR1..40
            CableDict[key][203] = Start of RTermination1..40
            CableDict[key][243] = Start of RSignal1..40
            CableDict[key][283] = Start of CoreNumber1..40
            CableDict[key][323] = CoreConfiguration e.g. '2Pr. 22 AWG OS'
            """
            if isinstance(obj,DS_BES_Cable):
                CableDict[key] = []
                CableDict[key].append(obj.Connection1)
            ...
            if isinstance(obj,DS_BES_Enclosure):
                ...
                for i in range(1,31):              
                    attribute = 'Connection'+str(i)
                    #print(obj.__dict__[attribute])
                    if obj.__dict__[attribute] != None:
                        EnclosureDict[key].append(obj.__dict__[attribute])
                    else:
                        EnclosureDict[key].append("SPARE")                     
        connection.close()
                    ...
                    if type(CableDict[item][1]) is list:
                        ...
                        if CableDict[item][1][2] == 'IS': # individual screen per pair or triad or quad
                            ...
                            for i in range(1,corenumber+1):                       
                                ...
                                Terminations.ActiveSheet.Cells(row,1).Name = LSignal
                                Terminations.ActiveSheet.Range("B"+str(row)).NumberFormat = "@"
                                Terminations.ActiveSheet.Cells(row,2).Value = CableDict[item][i+42]   # LTermination starts on index 43
                                Terminations.ActiveSheet.Cells(row,2).Name = LTermination
                                Terminations.ActiveSheet.Cells(row,3).Value = CableDict[item][i+282] #  CoreNumber starts at index 283 
                                Terminations.ActiveSheet.Cells(row,4).Value = CableDict[item][i+122]  # Color start at index 123
                                Terminations.ActiveSheet.Cells(row,4).Name = Color
                                Terminations.ActiveSheet.Cells(row,5).Value = CableDict[item][i+282] # CoreNumber starts at index 283
                                Terminations.ActiveSheet.Range("F"+str(row)).NumberFormat = "@"
                                Terminations.ActiveSheet.Cells(row,6).Value = CableDict[item][i+202]  # RTermination starts on index 203
                                Terminations.ActiveSheet.Cells(row,6).Name = RTermination
                                Terminations.ActiveSheet.Cells(row,7).Value = CableDict[item][i+242]  # RSignal starts on index 243
                                Terminations.ActiveSheet.Cells(row,7).Name = RSignal                            
                                row += 1
                                counter += 1
                                if counter == multiplier:
                                    ...
                                    LSCR = Name + "LSCR" + str(screennumber)
                                    RSCR = Name + "RSCR" + str(screennumber)
                                    Terminations.ActiveSheet.Cells(row,2).Value = CableDict[item][screennumber+82]   # LSCR starts on index 83
                                    Terminations.ActiveSheet.Cells(row,2).Name = LSCR
                                    Terminations.ActiveSheet.Cells(row,3).Value = "SCR"+str(screennumber)                            
                                    Terminations.ActiveSheet.Cells(row,5).Value = "SCR"+str(screennumber)
                                    Terminations.ActiveSheet.Cells(row,6).Value = CableDict[item][screennumber+162]  # RSCR starts on index 163
                                    Terminations.ActiveSheet.Cells(row,6).Name = RSCR
                                    row +=1
                                    counter = 0 # reset the multiplier counter
                        if CableDict[item][1][2] == 'OS' or CableDict[item][1][2] == 'PE':
